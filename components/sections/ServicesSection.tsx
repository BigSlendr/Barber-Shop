    ...
    <section>
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {services.map((service) => (
          <article key={service.id} className="card card-hover">
            <h3 className="text-xl font-semibold">{service.name}</h3>
            <p className="mt-2 text-sm text-brand-muted">{service.description}</p>
            <div className="mt-6 flex items-end justify-between">
              <p className="text-sm uppercase tracking-[0.08em] text-brand-muted">{service.durationMins} min</p>
              <p className="text-xl font-semibold text-brand-accent">{currency(service.price)}</p>
            </div>
          </article>
        ))}
      </div>
