...
export function BookingForm({ initialBarberId = "" }: BookingFormProps) {
  const [form, setForm] = useState<FormState>({
    barberId: initialBarberId || (activeShop.barbers[0]?.id ?? ""),
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({ ...prev, barberId: initialBarberId, time: "" }));
  }, [initialBarberId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setResult({
        ...
        message: `Request received for ${selectedBarber?.name ?? "your barber"} on ${form.date} at ${form.time}. We will confirm by phone or email shortly.`
      });
    } catch {
      setResult({ ok: false, error: "Unable to submit right now. Please call the shop directly." });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (result?.ok) {
    return (
      <div className="rounded-2xl border border-green-500/30 bg-green-500/5 p-8 text-center">
        ...
        <h2 className="text-2xl font-semibold text-white">Booking Request Sent!</h2>
        ...
        <p className="mt-2 text-xs text-brand-muted">
          Static site mode: connect this form to your preferred booking backend when rebranding.
        </p>
        <button
          type="button"
          onClick={() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <input type="email" className="field-input" placeholder="Email address (confirmation will be sent here)" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        <textarea className="field-input min-h-20" placeholder="Notes (optional - style references, allergies, etc.)" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
      </div>
      ...
      <div className="flex flex-col gap-3">
        <button type="submit" disabled={!form.time || submitting} className="btn-primary w-full disabled:cursor-not-allowed disabled:opacity-50">
          {submitting ? "Confirming..." : "Confirm booking"}
        </button>
        <p className="text-center text-xs text-brand-muted">
          Static template mode: this demo captures booking details on-page for GitHub Pages compatibility.
        </p>
      </div>
    </form>
  );
